...
all_markers_clustID <- presto::wilcoxauc(MEA_Glia, 'seurat_clusters', assay = 'data')

all_markers.Sign <- all_markers_clustID %>%
  dplyr::filter(padj < 0.05, logFC > 0)

openxlsx::write.xlsx(all_markers.Sign, 
                     file = "output_Glia/Presto_padjLT05_logfcGT0.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Markers")

markers <- readxl::read_excel("output_Glia/Presto_padjLT05_logfcGT0.xlsx")

markers_sign <- markers %>%
  dplyr::filter(padj < 0.05, pct_in > 20, pct_out < 20) %>% 
  dplyr::rename(Gene = feature, Cluster = group)

write.table(markers_sign,"output_Glia/Markers_Significant.txt",sep="\t",quote=F,row.names=F)

df <- markers_sign %>% dplyr::select(Gene,Cluster)
write.table(df,"output_Glia/Markers_Significant_DGE.txt",sep="\t",quote=F,row.names=F)
...
jormarkers <- readxl::read_excel("output_Glia/Jorstad_Glia_ACC_DLPFC.xlsx") 

Jordf <- jormarkers %>% dplyr::select(gene,cluster)
Jordf <- Jordf %>% dplyr::rename(Gene = gene, Cluster = cluster) 
head(Jordf)
write.table(Jordf,"output_Glia/FE_Jorstad_Glia_ACC_DLPFC_Markers_Significant.txt",sep="\t",quote=F,row.names=F)

jorlist <- split(Jordf,f = Jordf$Cluster)
save(jorlist, file ="output_Glia/PFC_Glia_ACC_DLPFC_GeneSets_FisherEnrichment.RData")
...
wgcna = list.files(path = "output_Glia",pattern = 'Markers_Significant_DGE.txt',full.names = TRUE)
...
load("output_Glia/PFC_Glia_ACC_DLPFC_GeneSets_FisherEnrichment.RData")#trying with ACC and PFC
ln=length(jorlist)
...
write.table(tmp2,"output_Glia/Jorstad_Labels_Clusters_ACC&PFC.txt",sep="\t",quote=F)
...
pdf("output_Glia/04_Jorstad_Enrichment_ACC&PFC.pdf",width=18,height=8)
ggscatter(p, 
          x = "variable",
          y = "Module",
          size="OR",
