...
all_markers_clustID <- presto::wilcoxauc(MEA_Inh, 'seurat_clusters', assay = 'data')

all_markers.Sign <- all_markers_clustID %>%
  dplyr::filter(padj < 0.05, logFC > 0)

openxlsx::write.xlsx(all_markers.Sign, 
                     file = "output_InhNeu/Presto_padjLT05_logfcGT0.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Markers")

markers <- readxl::read_excel("output_InhNeu/Presto_padjLT05_logfcGT0.xlsx")

markers_sign <- markers %>%
  dplyr::filter(padj < 0.05, pct_in > 20, pct_out < 20) %>% 
  dplyr::rename(Gene = feature, Cluster = group)

write.table(markers_sign,"output_InhNeu/Markers_Significant_Res06.txt",sep="\t",quote=F,row.names=F)

df <- markers_sign %>% dplyr::select(Gene,Cluster)
write.table(df,"output_InhNeu/Markers_Significant_DGE_Res06.txt",sep="\t",quote=F,row.names=F)
...
jormarkers <- readxl::read_excel("output_InhNeu/Jorstad_Inh_ACC_DLPFC.xlsx") 

Jordf <- jormarkers %>% dplyr::select(gene,cluster)
Jordf <- Jordf %>% dplyr::rename(Gene = gene, Cluster = cluster) 
head(Jordf)
write.table(Jordf,"output_InhNeu/FE_Jorstad_Inh_ACC_DLPFC_Markers_Significant.txt",sep="\t",quote=F,row.names=F)

jorlist <- split(Jordf,f = Jordf$Cluster)
save(jorlist, file ="output_InhNeu/PFC_Inh_ACC_DLPFC_GeneSets_FisherEnrichment.RData")
...
wgcna = list.files(path = "output_InhNeu",pattern = 'Markers_Significant_DGE_Res04.txt',full.names = TRUE)
...
load("output_InhNeu/PFC_Inh_ACC_DLPFC_GeneSets_FisherEnrichment.RData")#trying with ACC and PFC
ln=length(jorlist)
...
write.table(tmp2,"output_InhNeu/Jorstad_Labels_Clusters_ACC&PFC.txt",sep="\t",quote=F)
...
#p$variable <- factor(p$variable,levels=c("Micro_L1-3","Endo_L2-6","Astro_L1-2", "Astro_L1-6","OPC_L1-6","Oligo_L1-6","Inh_L2","Inh_L2-3","Inh_L2-4","Inh_L3-4","Inh_L3-5","Inh_L4-5","Inh_L4-6","Inh_L5-6",
#                                          "Inh_L6","Inh_L1","Inh_L1-2","Inh_L1-3","Inh_L1-4","Inh_L2-3","Inh_L2-4", "Inh_L2-5","Inh_L2-6","Inh_L3-5","Inh_L3-6","Inh_L4-5","Inh_L4-6","Inh_L5-6"))

pdf("output_InhNeu/04_Jorstad_Enrichment_ACC&PFC.pdf",width=18,height=8)
ggscatter(p, 
          x = "variable",
          y = "Module",
          size="OR",
