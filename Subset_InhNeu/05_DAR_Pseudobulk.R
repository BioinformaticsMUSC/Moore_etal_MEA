...
load("output_InhNeu_New/03_InhNeurons_Res05_NewLabel.RData")

MEA_Inh
#An object of class Seurat 
#513527 features across 7992 samples within 6 assays 
#Active assay: macpeaks (201661 features, 50468 variable features)
...
#f1 <- sapply(strsplit(as.character(rownames(cts.t)), split = "_"), FUN = "[", 1)
#f2 <- sapply(strsplit(as.character(rownames(cts.t)), split = "_"), FUN = "[", 2)
#splitrows <- paste(f1, f2, sep = "_")
#added new column with - instead _ so nomore confusion
...
dir.create('output_InhNeu_New/DAR_Pseudobulk_WALD_Signac')

write.table(metadata,'output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/Pseudobulk_Meta.txt',
            sep="\t",quote=F,row.names=T)

save(cts.split.mod,metadata, file = "output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/PseudobulkMatrix.RData")
...
purrr::iwalk(FullTabStats, ~saveRDS(.x, file = paste0("output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/",.y, "_FullStats.RDS")))
purrr::iwalk(DGETabStats, ~saveRDS(.x, file = paste0("output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/",.y, "_DGEStats.RDS")))


purrr::iwalk(FullTabStats, ~openxlsx::write.xlsx(.x, file = paste0("output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/", .y, "_FullStats.xlsx"), colNames=T,rowNames=F,boarders="columns"))
purrr::iwalk(DGETabStats, ~openxlsx::write.xlsx(.x, file = paste0("output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/", .y, "_DGEStats.xlsx"), colNames=T,rowNames=F,boarders="columns"))

openxlsx::write.xlsx(do.call(rbind, FullTabStats), 
                     file = "output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/AllCellCombined_FullStats.xlsx",
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Stats")
...
openxlsx::write.xlsx(do.call(rbind, DGETabStats), 
                     file = "output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/AllCellCombined_DGEStats.xlsx", 
                     colNames = TRUE,
                     rowNames = FALSE, 
                     borders = "columns",
                     sheetName="Stats")
...
pdf("output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/01_Barplot_Number_Signac_DARs.pdf",width = 4,height = 4)
...
pdf("output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/02_Barplot_Number_Signac_DARs_Direction.pdf",width = 6,height = 4)
...
tmp1 <- readxl::read_excel("output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/AllCellCombined_DGEStats.xlsx",sheet = "Stats")
head(tmp1)
...
write.table(closest_genes_darStim,"output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/ClosestGene_Stim.txt",
            sep="\t",quote=F,row.names=F)

write.table(closest_genes_darCtrl,"output_InhNeu_New/DAR_Pseudobulk_WALD_Signac/ClosestGene_Ctrl.txt",
            sep="\t",quote=F,row.names=F)
